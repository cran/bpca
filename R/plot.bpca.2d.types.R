...
.plot_type_eo <- function(ctx, ...) {
  ...
  points(
    x = ctx$coobj[obj.id[1], ctx$d1] * ctx$obj.factor,
...
.plot_type_ev <- function(ctx, ...) {
  ...
  points(
    x = ctx$covar[var.id, ctx$d1] * ctx$var.factor,
...
.plot_type_ms <- function(ctx, ...) {
  ...
  for (i in seq_len(nrow(ctx$coobj))) {
    x.int <- .solve_orthogonal_intersection(
      px = ctx$coobj[i, ctx$d1],
