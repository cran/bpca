...
plot(bp1, var.factor=.3,
     obj.names=FALSE, obj.pch=c('+', '-', '*')[unclass(iris$Species)],
...
plot(bpca(iris[-5], lambda.ini=3, lambda.end=4), var.factor=.6,
     obj.names=FALSE, obj.pch=c('+', '-', '*')[unclass(iris$Species)],
     obj.col=c('red', 'green3', 'blue')[unclass(iris$Species)], obj.cex=1)
...
bp2 <- bpca(iris[-5], lambda.end=3, var.rb=TRUE, var.rd=TRUE, limit=2)

plot(bp2, obj.names=FALSE, 
