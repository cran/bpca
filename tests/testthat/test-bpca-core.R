...
test_that("bpca returns the expected classes and components", {
  ...
  expect_true(all(c(
    "coord", "importance", "eigenvalues",
    "eigenvectors", "number", "var.rb", "var.rd"
  ) %in% names(bp2)))
...
test_that(".center_scale: center=0 returns the matrix unchanged", {
  x <- matrix(1:9, nrow = 3)
  expect_equal(
    bpca:::.center_scale(x, center = 0, scale = FALSE),
    x
  )
})
...
test_that(".compute_var_factor: error when all variable coordinates are zero", {
  ...
  expect_error(
    bpca:::.compute_var_factor(coobj, covar),
